...
def best_bd_site(fname, lat, lon, height):
    """
        ...
        for row in csv_reader:
            ...
            if (row[1][:4] == 'LAT=' and row[2][:4] == 'LON=' and
                ...
                if len(row[1]) > 4 and len(row[2]) > 4 and len(row[4]) > 4:
                    ...
                    if (dist_from_site < best_site[1] and
                            abs(height - bd_height) < 50):
                        best_site = [row[3][10:], dist_from_site, bd_height]
                        best_row = row
